...
const process = [["01", "Audit", "We look closely at what’s working, what’s missing, and where the next meaningful signal lives."], ["02", "Strategy", "A clear creative and growth roadmap your whole team can understand — and use."], ["03", "Create", "We make the work: visual systems, stories, campaigns, and content built for the channel."], ["04", "Grow", "We launch, learn, and compound what works with a rhythm that keeps momentum honest."]];
...
export default function Services() {
  return <PageShell><main><PageIntro eyebrow="Capabilities" title={<>Make good work <span className="italic text-[#ffbf00]">matter.</span></>} description="Strategy, social media, content creation, and digital marketing aligned toward one clear outcome: a brand people notice, trust, and choose." />
    ...
    <section className="border-y border-[#e8e1ce] bg-[#f7f1e3]" data-testid="services-process-section"><div className="mx-auto max-w-[1320px] px-5 py-24 lg:px-8 lg:py-32"><Eyebrow>How it comes together</Eyebrow><div className="mt-12 grid gap-0 border-l border-[#e8e1ce] lg:grid-cols-4 lg:border-l-0 lg:border-t">{process.map(([number, title, text]) => <div key={number} className="relative border-b border-[#e8e1ce] py-8 pl-8 lg:border-b-0 lg:border-l lg:pl-6 lg:pt-8" data-testid={`process-step-${number}`}><span className="absolute -left-[5px] top-9 size-2 rounded-full bg-[#ffbf00] lg:-top-[5px] lg:left-6" /><p className="font-mono text-xs text-[#ffbf00]">{number}</p><h3 className="mt-8 font-serif text-3xl">{title}</h3><p className="mt-3 text-sm leading-6 text-[#575247]">{text}</p></div>)}</div></div></section>
