...
const questions = [
  ["Working together", "How quickly can we get started?", "Most partnerships begin with a two-week audit and strategy sprint. From there, we move into a rhythm that fits the shape of your team and your goals."],
  ["Working together", "Do you work with in-house marketing teams?", "Always. We’re designed to add senior perspective and creative capacity without adding another layer of process."],
  ["Investment", "What does a typical engagement cost?", "Our monthly partnerships typically begin around $3k and grow with the depth of strategy, content, and media support needed."],
  ["Investment", "Are there long-term contracts?", "We ask for enough runway to make meaningful work — usually three months — then keep the relationship practical and transparent."],
  ["The work", "Can you work with our existing brand identity?", "Yes. We can extend a strong system, sharpen a fuzzy one, or create a new visual language when the moment calls for it."],
  ["The work", "Who owns the content you create?", "You do. Every asset, insight, and piece of thinking created for your brand belongs with your team."],
];
const categories = ["All", "Working together", "Investment", "The work"];

export default function FAQ() {
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState("All");
  const [open, setOpen] = useState<number | null>(0);
  const filtered = useMemo(() => questions.filter(([group, question]) => (category === "All" || group === category) && `${group} ${question}`.toLowerCase().includes(query.toLowerCase())), [category, query]);
